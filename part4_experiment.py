...
def naive_bayes(emission_matrix, hidden_states: list, total_count_per_state: dict):
    ...
    mean = np.mean(list(hidden_state_probability.values()))
    var = np.var(list(hidden_state_probability.values()))
    
    # get probability of each state, log final vgitalue to avoid underflow -> p(y)
    for k,v in hidden_state_probability.items():
        hidden_state_probability[k] = math.log(normal(v,mean,var))
...
def write_to_file(lang, states, words):
    with open(f'{lang}/dev.p4_ex.out', 'w', encoding='utf-8') as f:
...
if __name__ == '__main__':
    ...
    # os.system(f"python3 ./EvalScript/evalResult.py ./{lang}/dev.out ./{lang}/dev.p4_ex.out")